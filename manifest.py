manifest = [{
    "filename": "bear.jpeg",
    "description":
    "A massive bear stands tall, its powerful presence filling you with both awe and fear. You tread cautiously, knowing any sudden movement could provoke its fierce wrath.",
    "on_discover": {
        "description": "Best not to linger (Courage -2)",
        "stats": [{
            "stat": "courage",
            "diff": -2
        }]
    },
    "on_return": {
        "description":
        "The bear is still here, or has it been following you? (Courage -4)",
        "stats": [{
            "stat": "courage",
            "diff": -4
        }]
    }
}, {
    "filename": "big trees.jpeg",
    "description":
    "Towering ancient trees surround you, their branches reaching for the heavens. As you walk beneath their majestic canopy, you feel a sense of ancient wisdom and tranquility.",
    "on_discover": {
        "description": "Embraced by nature (Vigor +1)",
        "stats": [{
            "stat": "vigor",
            "diff": 1
        }]
    },
    "on_return": {
        "description": "You feel small and alone now (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    }
}, {
    "filename": "campfire2.png",
    "description":
    "A crackling campfire illuminates the darkness, casting dancing shadows on the surrounding trees. The warmth and flickering glow invite you to sit and gaze up at the starry sky.",
    "on_discover": {
        "description": "Regroup near the fire (Vigor +2)",
        "stats": [{
            "stat": "vigor",
            "diff": 2
        }]
    },
    "on_return": {
        "description": "The embers have died down somewhat (Vigor +1)",
        "stats": [{
            "stat": "vigor",
            "diff": 1
        }]
    }
}, {
    "filename": "campfire.jpg",
    "description":
    "You approach a campfire, its gentle flames dancing in the night. The comforting glow and the crackling of burning logs are a welcome sight.",
    "on_discover": {
        "description": "Warm up by the fire (Vigor +1)",
        "stats": [{
            "stat": "vigor",
            "diff": 1
        }]
    },
    "on_return": {
        "description": "How many campfires are there? (Vigor +1)",
        "stats": [{
            "stat": "vigor",
            "diff": 1
        }]
    }
}, {
    "filename": "crows.jpeg",
    "description":
    "The forest grows dark. You hear a fluttering overhead. When you turn, ominous crows stare down at you with beady eyes. Your throat tightens, your mouth goes dry.",
    "on_discover": {
        "description": "Intimidating crows (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    },
    "on_return": {
        "description": "You are going in circles! (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    }
}, {
    "filename": "foggy path.jpeg",
    "description":
    "A foggy path winds through the dense forest, its misty tendrils concealing secrets and uncertainty. As you tread softly, you sense a hidden presence observing your every move.",
    "on_discover": {
        "description": "Navigating the fog (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    },
    "on_return": {
        "description": "Are you lost? (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    }
}, {
    "filename": "hills and path.jpg",
    "description":
    "You ascend the hills, the path stretching before you like an adventure waiting to unfold. With each step, excitement fills your chest, and you can't help but wonder what awaits at the summit.",
    "on_discover": {
        "description": "Challenging the hills (Courage +2, Vigor -1)",
        "stats": [{
            "stat": "courage",
            "diff": 2
        }, {
            "stat": "vigor",
            "diff": -1
        }]
    },
    "on_return": {
        "description": "Been there, done that (Vigor -1)",
        "stats": [{
            "stat": "vigor",
            "diff": -1
        }]
    }
}, {
    "filename": "hut1.png",
    "description":
    "A small, secluded hut nestled amidst the wilderness beckons you for shelter. Its humble charm and rustic allure offer respite from the trials of the wild, whispering tales of wanderers past.",
    "on_discover": {
        "description": "Seeking shelter (Vigor +5)",
        "stats": [{
            "stat": "vigor",
            "diff": 5
        }]
    },
    "on_return": {
        "description": "Happy to rest (Vigor +5)",
        "stats": [{
            "stat": "vigor",
            "diff": 5
        }]
    }
}, {
    "filename": "lamp post.jpg",
    "description":
    "A solitary lamp post stands tall in the darkness, casting a dim glow on the surrounding area. Inspired by this symbol of hope, you find solace and determination to continue on your journey.",
    "on_discover": {
        "description": "Guided by the light (Courage +2)",
        "stats": [{
            "stat": "courage",
            "diff": 2
        }]
    },
    "on_return": {
        "description": "Back rain started (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    }
}, {
    "filename": "meadow3.jpg",
    "description":
    "You step into a vast, sun-drenched meadow teeming with vibrant wildflowers. The sweet scent of blooming blossoms fills the air, and you find peace in the embrace of nature's untouched beauty.",
    "on_discover": {
        "description": "Basking in nature's beauty (Courage +2, Vigor +2)",
        "stats": [{
            "stat": "courage",
            "diff": 2
        }, {
            "stat": "vigor",
            "diff": 2
        }]
    },
    "on_return": {
        "description": "A moment's rest (Vigor +1)",
        "stats": [{
            "stat": "vigor",
            "diff": 1
        }]
    }
}, {
    "filename": "meadow4.jpg",
    "description":
    "A breathtaking meadow stretches before you, a symphony of colors beneath the vast blue sky. As you walk through this enchanting oasis, you feel as though the worries of the world melt away.",
    "on_discover": {
        "description": "Serene meadow (Vigor +2, Courage +2)",
        "stats": [{
            "stat": "vigor",
            "diff": 2
        }, {
            "stat": "courage",
            "diff": 2
        }]
    },
    "on_return": {
        "description": "what are you ever find what you were looking for?",
        "stats": []
    }
}, {
    "filename": "path1.jpg",
    "description":
    "A narrow path meanders through the wilderness, leading you to discover hidden landscapes and untold stories. With every twist and turn, anticipation builds.",
    "on_discover": {
        "description": "How long will this journey be?",
        "stats": []
    },
    "on_return": {
        "description": "The winding road goes on and on (Vigor -1)",
        "stats": [{
            "stat": "vigor",
            "diff": -1
        }]
    }
}, {
    "filename": "pond night.jpeg",
    "description":
    "By the tranquil pond, the moon's gentle reflection dances on the rippling water. In the stillness of the night, a sense of serenity envelops you, as if the world exists solely within this peaceful realm.",
    "on_discover": {
        "description": "Tranquil pond (Courage +1)",
        "stats": [{
            "stat": "courage",
            "diff": 1
        }]
    },
    "on_return": {
        "description": "You've been here already, right?",
        "stats": []
    }
}, {
    "filename": "pond raining.jpeg",
    "description":
    "Raindrops pelt the surface of a desolate pond. The air is heavy with a chill, and the darkness is intensified by the downpour.",
    "on_discover": {
        "description": "Cold and wet (Courage 11, Vigor -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }, {
            "stat": "vigor",
            "diff": -1
        }]
    },
    "on_return": {
        "description":
        "Still raining, still cold, still wet (Courage -1, Vigor -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }, {
            "stat": "vigor",
            "diff": -1
        }]
    }
}, {
    "filename": "river1.jpg",
    "description":
    "A mighty river rushes past with unrestrained force, blocking your path. You'll have to go around",
    "on_discover": {
        "description": "Roaring river (Vigor -3)",
        "stats": [{
            "stat": "vigor",
            "diff": -3
        }]
    },
    "on_return": {
        "description":
        "You thought you found a way around this.  You were wrong. (Vigor -1)",
        "stats": [{
            "stat": "vigor",
            "diff": -1
        }]
    }
}, {
    "filename": "wolves.png",
    "description":
    "You hear howling echoing through the trees. The eerie sound gets closer. This is not good.",
    "on_discover": {
        "description": "Prey (Courage -4)",
        "stats": [{
            "stat": "courage",
            "diff": -4
        }]
    },
    "on_return": {
        "description": "Why did you come back here? (Courage -1)",
        "stats": [{
            "stat": "courage",
            "diff": -1
        }]
    }
}]
